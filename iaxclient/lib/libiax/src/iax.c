...
#define MYSNPRINTF snprintf(requeststr + strlen(requeststr), sizeof(buf) - sizeof(struct iax_full_hdr) - strlen(requeststr), 
	...
	case IAX_EVENT_CONNECT:
		...
		if (event->event.connect.username)
			strncpy(session->username, event->event.connect.username, sizeof(session->username) - 1);
		else
			strcpy(session->username, "");
		if (event->event.connect.secret)
			strncpy(session->secret, event->event.connect.secret, sizeof(session->secret) - 1);
		else
	...
	if (e) {
		...
		case AST_FRAME_IAX:
			...
			case IAX_COMMAND_AUTHREQ:
				...
				if (strlen(session->username) && !strcmp(e->event.authrequest.username, session->username) &&
					strlen(session->secret)) {
						/* Hey, we already know this one */
						iax_auth_reply(session, session->secret, e->event.authrequest.challenge, e->event.authrequest.authmethods);
						if (e->event.authrequest.username)
							free(e->event.authrequest.username);
						if (e->event.authrequest.challenge)
							free(e->event.authrequest.challenge);
						free(e);
						return NULL;
				}
				return schedule_delivery(e, ts);
			...
			case IAX_COMMAND_TXREQ:
				...
				if (s) {
					int ret=0;
					ret=inet_aton(s, &session->transfer.sin_addr);
					free(s);
					if(ret){
			...
			case IAX_COMMAND_DPREP:
				...
				if (s) {
					...
					else e->event.dprep.ignorepat=0;
					free(s);
				}
...
void iax_event_free(struct iax_event *event)
{
	...
	case IAX_EVENT_CONNECT:
		...
		if (event->event.connect.secret)
			free(event->event.connect.secret);
		break;
	case IAX_EVENT_HANGUP:
		if (event->event.hangup.byemsg)
